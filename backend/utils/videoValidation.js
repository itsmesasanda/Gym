...
export const isHttpUrl = (value) => Boolean(parseHttpUrl(value));
...
export const normalizeThumbnail = (thumbnail, youtubeUrl) => {
  const trimmedThumbnail = (thumbnail || "").trim();

  if (trimmedThumbnail) {
    const thumbnailVideoId = extractYouTubeVideoId(trimmedThumbnail);
    if (thumbnailVideoId) return `https://img.youtube.com/vi/${thumbnailVideoId}/hqdefault.jpg`;
    if (isDirectImageUrl(trimmedThumbnail)) return trimmedThumbnail;
  }
