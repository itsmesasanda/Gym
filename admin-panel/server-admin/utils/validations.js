...
const validatePhone = (phone) => {
  ...
  return {
    isValid,
    error: isValid ? null : 'Phone must be exactly 10 digits',
  };
};
...
/**
 * Validate user data - all required fields
 * @param {object} user
...
const validateUserData = (user) => {
  ...
  const nameCheck = validateRequired(user.name, 'Name');
...
/**
 * Validate announcement data - required fields
 * @param {object} announcement
 * @returns {object} { isValid, errors }
 */
const validateAnnouncementData = (announcement) => {
  const errors = {};
...
/**
 * Validate event data - required fields
 * @param {object} event
 * @returns {object} { isValid, errors }
 */
