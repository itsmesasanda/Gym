...
export default function Members() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const { data } = await api.get("/api/admin/members", { params: { status, search } });
      setMembers(data);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div>
                ...
                <tr key={m._id}>
                  ...
                  <td>
                    <div className="row-actions">
                      <button className="btn btn-sm btn-outline" onClick={() => setQrMemberId(m._id)}>QR</button>
                      {actionsFor(m)}
                    </div>
                  </td>
                </tr>
      ...
      <MemberFormModal
        ...
        onCreated={() => { setShowAdd(false); setStatus("all"); load(); }}
      />
      <MemberQrModal memberId={qrMemberId} onClose={() => setQrMemberId(null)} />
    </div>
  );
}
